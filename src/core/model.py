...
class DecomposedDense(nn.Module):
    def __init__(self,
        ...
        self.sw = nn.Parameter(shared.clone().detach(), requires_grad=True) if shared is not None else None
        self.aw = nn.Parameter(adaptive.clone().detach(), requires_grad=True) if adaptive is not None else None
        self.mask = nn.Parameter(mask.clone().detach(), requires_grad=True) if mask is not None else None
        self.bias = nn.Parameter(bias.clone().detach(), requires_grad=True) if bias is not None else None
        ...
        self.atten = nn.Parameter(atten.clone().detach(), requires_grad=True) if atten is not None else None
...
class SimpleCNN(nn.Module):
    ...
    FC(256 -> num_classes)
    """
    def __init__(self, args, num_classes=10, in_channels=3):
        super().__init__()
        if args is None:
    ...
    def init_decomposed_variables(self, initial_weights):
        self.decomposed_variables['shared'] = [
            nn.Parameter(torch.tensor(initial_weights[i], dtype=torch.float32), requires_grad=True)
    ...
    def create_variable(self, var_type, lid, tid=0):
        ...
        if var_type == 'adaptive':
            ...
            if len(self.shapes[lid]) == 3:
                shape = (1, self.shapes[lid][1], self.shapes[lid][0], self.shapes[lid][2], self.shapes[lid][2])
            else:
            ...
            if len(self.shapes[lid]) == 3:
                shape = (self.shapes[lid][1], self.shapes[lid][0], self.shapes[lid][2], self.shapes[lid][2])
            else:
